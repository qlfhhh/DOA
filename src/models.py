...
class ModelGenerator(object):
    ...
    def set_diff_method(self, diff_method: str = "root_music"):
        ...
        """
        if self.model_type.startswith("SubspaceNet"):
            if diff_method not in ["esprit", "root_music", "toeplitz"]:
                raise ValueError(
...
class SubspaceNet(nn.Module):
    ...
    def set_diff_method(self, diff_method: str):
        ...
        elif diff_method.startswith("toeplitz"):
            self.diff_method = toeplitz
        else:
            raise Exception(
...
def toeplitz(Rz: torch.Tensor, K: int,batch_size: int,N=8):
    """Implementation of the model-based Esprit algorithm, support Pytorch, intended for
        MB-DL models. the model sets for nominal and ideal condition (Narrow-band, ULA, non-coherent)
        as it accepts the surrogate covariance matrix.
        it is equivalent to src.methods: RootMUSIC.narrowband() method.
    ...
    for iter in range(batch_size):
        CovMatix = Bs_Rz[iter]
        m = int(np.ceil((N + 1) / 2))
        CovariceNew = torch.zeros((m, m))
        J = torch.fliplr(torch.eye(m, m))
        for i in range(CovMatix.shape[0]):
            row = CovMatix[m - 1:, i]
            col = torch.flip(CovMatix[:m, i],[0])
            R = scipy.linalg.toeplitz(col.detach().numpy(), row.detach().numpy())
            R = torch.tensor(R).float()
            CovariceNew = CovariceNew + R @ torch.conj(torch.transpose(R, 0, 1))
        R = CovariceNew + J @ torch.conj(CovariceNew) @ J
        [U, S, V] = torch.linalg.svd(R)
        Us = U[:, :K]
        Us1 = Us[:-1, :]
        Us2 = Us[1:, :]
        Us12 = torch.concatenate((Us1, Us2), 1)
        [E, Sa, Va] = torch.linalg.svd(torch.conj(torch.transpose(Us12, 0, 1)) @ Us12)
        E11 = E[:K, :K]
        E12 = E[:K, K:2 * K]
        E21 = E[K:2 * K, :K]
        E22 = E[K:2 * K, K:2 * K]
        M = -(E12 @ torch.linalg.inv(E22))
        Dm, Vm = torch.linalg.eig(M)
        eigenvalues_angels = torch.angle(Dm)
